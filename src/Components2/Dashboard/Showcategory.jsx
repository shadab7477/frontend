...
const Showcategory = () => { const [products, setProducts] = useState([]);
    ...
    <div className="maintable bg-white mt-4 pt-3 px-3 rounded-3 ">
        <div className="row">
        <span className="fs-1 col-6 pt-2" >Category list</span>
      ...
      />
        ...
        <tbody>
          {products.map(product => (
            <tr key={product._id}>
              <td>{product.name}</td>
              <td><img src={`http://localhost:9000/category/${product.image}`} width="100" height="80" /></td>
